...
autostart(void)
{
	...
	/* if $XDG_DATA_HOME is set and not empty, use $XDG_DATA_HOME/swm,
	 * otherwise use ~/.local/share/swm as autostart script directory
	 */
	xdgdatahome = getenv("XDG_DATA_HOME");
	if (xdgdatahome != NULL && *xdgdatahome != '\0') {
		/* space for path segments, separators and nul */
		pathpfx = ecalloc(1, strlen(xdgdatahome) + strlen(cfgdir) + 2);

		if (sprintf(pathpfx, "%s/%s", xdgdatahome, cfgdir) <= 0) {
			free(pathpfx);
			return;
		}
	} else {
		...
		if (sprintf(pathpfx, "%s/%s/%s", home, localshare, cfgdir) < 0) {
			free(pathpfx);
			return;
		}
	}
	...
	if (!(stat(pathpfx, &sb) == 0 && S_ISDIR(sb.st_mode))) {
		 ...
		 */
		char *pathpfx_new = realloc(pathpfx, strlen(home) + strlen(cfgdir) + 3);
		if(pathpfx_new == NULL) {
			free(pathpfx);
			return;
		}
